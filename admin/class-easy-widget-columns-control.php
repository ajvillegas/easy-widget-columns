...
class Easy_Widget_Columns_Control {
	...
	public function widget_form( $widget, $return, $instance ) {
		...
		if ( ! empty( $ewc_include_widgets )
			&& ( in_array( $widget->id_base, array( 'ewc-row-divider', 'ewc-subrow-divider' ), true )
			|| ! in_array( $widget->id_base, $ewc_include_widgets, true ) ) ) {
			return;
		} elseif ( empty( $ewc_include_widgets )
			&& ( in_array( $widget->id_base, array( 'ewc-row-divider', 'ewc-subrow-divider' ), true )
			|| in_array( $widget->id_base, $ewc_exclude_widgets, true ) ) ) {
			return;
		}

		// Add custom select control.
		$defaults = array(
			'ewc_width' => esc_html__( 'none', 'easy-widget-columns' ),
		);
	...
	public function widget_update( $instance, $new_instance, $old_instance ) {
		...
		$instance['ewc_width'] = wp_strip_all_tags( $new_instance['ewc_width'] );
	...
	/**
	 * Get the widget's ID number.
	 *
	 * @since 1.0.0
	 *
	 * @param string $widget The widget ID name.
	 *
	 * @return array
	 */
	 ...
	 *
	 * @since 1.1.0
	 * @param array $params The widget sidebar parameters.
	 *
	 * @return array
	 */
	public function sidebar_params( $params ) {
		...
		$position_id      = array_search( $params[0]['widget_id'], $sidebar_id, true );
		...
		if ( ( isset( $sidebar_id[ $position_id - 1 ] )
			&& in_array( _get_widget_id_base( $sidebar_id[ $position_id - 1 ] ), array( 'ewc-row-divider' ), true ) )
			|| ( ( isset( $sidebar_id[ $position_id - 1 ] )
				&& isset( $sidebar_id[ $position_id - 2 ] )
				&& in_array( _get_widget_id_base( $sidebar_id[ $position_id - 1 ] ), array( 'ewc-subrow-divider' ), true )
		...
		// Output the column classes and markup.
		if ( ( ! empty( $ewc_width ) && 'none' !== $ewc_width )
			&& ( ! empty( $params[0]['before_widget'] ) && ! empty( $params[0]['after_widget'] ) ) ) {
