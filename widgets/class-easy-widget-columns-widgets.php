...
class Easy_Widget_Columns_Widgets {
	...
	private function load_widget_classes() {
		 ...
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'widgets/class-ewc-row-divider.php';

		/**
		 * The class responsible for defining the Sub Row widget.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'widgets/class-ewc-subrow-divider.php';
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_localize_script(
			...
			'ajv_image_upload',
			array(
				'frame_title'  => esc_html__( 'Choose or Upload Image', 'easy-widget-columns' ),
				'frame_button' => esc_html__( 'Insert Image', 'easy-widget-columns' ),
			)
		);
			...
			( function( $ ) {
				function initColorPicker( widget ) {
					widget.find( '.color-picker' ).wpColorPicker( {
						palettes: <?php echo wp_kses_post( $palettes ); ?>,
						width: 232,
	...
	public function register_widgets() {
		...
		register_widget( 'EWC_Subrow_Divider' );
